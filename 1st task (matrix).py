...
# Ниже я привел код распараллеливания для простой функции сна
import time
...
print("Время расчета :      ", timeit(lambda: [sl(x) for x in [1,2,3,4,5,6]], globals=globals() , number=n_iterations)) 

pool_2 =  multiprocessing.Pool(2)
print("Время расчета 4-ый пункт мульт:      ", timeit(lambda: pool_2.map(sl, [1,2,3,4,5,6]), number=n_iterations))  

pool_4 =  multiprocessing.Pool(4)
print("Время расчета 4-ый пункт мульт:      ", timeit(lambda: pool_4.map(sl, [1,2,3,4,5,6]), number=n_iterations))  

pool_6 =  multiprocessing.Pool(6)
print("Время расчета 4-ый пункт мульт:      ", timeit(lambda: pool_6.map(sl, [1,2,3,4,5,6]), number=n_iterations))
